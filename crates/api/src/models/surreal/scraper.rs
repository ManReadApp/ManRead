...
use crate::{
    error::ApiResult,
    init::db::DB,
    models::{chapter::ChapterDBService, manga::Manga, tag::Empty, version::Version},
};
...
impl ScraperDbService {
    pub async fn next_chapter(&self) -> ApiResult<Option<RecordData<ScraperEntry>>> {
    ...
    ) -> ApiResult<Vec<RecordData<ScraperEntry>>> {
        let v: Vec<RecordData<ScraperEntry>> = DB
            .query(format!(
    ...
    pub async fn process_items(
        &self,
    ) -> ApiResult<Vec<(RecordIdType<Manga>, RecordIdType<Version>)>> {
        let query = format!(
...
#[derive(Default, Serialize, Deserialize, Debug, Clone, JsonSchema, ApiComponent)]
pub enum State {
    #[default]
    Pending,
    Declined,
