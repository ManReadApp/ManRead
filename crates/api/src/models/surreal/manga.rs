...
use crate::{
    ...
    init::db::DB,
    models::chapter::ChapterDBService,
};
...
impl MangaDBService {
    pub async fn scrapers(&self) -> ApiResult<Vec<RecordIdType<Manga>>> {
    ...
    pub async fn exists(&self, id: &str) -> ApiResult<()> {
    ...
    pub async fn add(&self, manga: Manga) -> ApiResult<RecordIdType<Manga>> {
        manga
            .add(&*DB)
            .await?
    ...
    pub async fn search(
        ...
        count: bool,
    ) -> ApiResult<(u64, Vec<RecordData<Manga>>)> {
        let mut what = vec!["*"];
        ...
        let query_ = match filter_array(data.query) {
            Some(arr) => format!(
                ...
                generate_array(arr, &user)
                    .map_err(ApiError::SearchParseError)?
                    .0
            ),
            None => "".to_owned(),
        };
    ...
    pub async fn regenerate_tags(&self, id: &str) -> ApiResult<()> {
        let id: RecordIdType<Manga> = RecordIdType::from((Manga::name(), id));
        let info = id.clone().get(&*DB).await?.ok_or(ApiError::NotFoundInDB)?;
    ...
    pub async fn set_visibility(&self, id: String, visibility: Visibility) -> ApiResult<()> {
    ...
    pub async fn get(&self, id: &str) -> ApiResult<Manga> {
