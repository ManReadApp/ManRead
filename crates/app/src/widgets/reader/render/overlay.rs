use egui::{Button, Color32, CursorIcon, Label, pos2, Rect, Response, Rounding, Sense, Ui, vec2, Widget};
...
impl Widget for Overlay {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        ui.painter().rect_filled(rect, Rounding::ZERO, Color32::from_rgba_unmultiplied(0,0,0,170));
        ui.horizontal_centered(|ui|{
            ...
            if home.clicked() {
                get_app_data().change(Page::Home, vec![Page::Reader { manga_id: "".to_string(), chapter_id: None }])
            }
            ui.style_mut().visuals.override_text_color = Some(grey);
            ui.add(Label::new(">").selectable(false));
            ui.style_mut().visuals.override_text_color = Some(white);
            let name =  ui.add(Label::new("One Piece").selectable(false).sense(Sense::click()));
            if name.hovered() {
                ui.ctx().set_cursor_icon(CursorIcon::PointingHand);
            }
            if name.clicked() {
                get_app_data().change(Page::MangaInfo("".to_string()), vec![Page::Reader { manga_id: "".to_string(), chapter_id: None }])
            }
            ui.style_mut().visuals.override_text_color = Some(grey);
            ui.add(Label::new(">").selectable(false));
            ui.style_mut().visuals.override_text_color = Some(white);

            ui.add(Label::new(match self.episode_name {
