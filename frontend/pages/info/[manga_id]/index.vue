...
useHead({
  title: (error.value ? 'Manga does not exist' : (value.value ? getTitle(value.value.titles) : "Manga Details Loading")) + " - ManRead",
})
//TODO: add relations

const img_url = computed(() => {
  return value.value ? getCoverUrl(value.value.manga_id, value.value.cover_ext[0] ?? '') : '';
});
...
const creator_string = computed(() => {
  if (!value.value) return "";
  return [...new Set([...value.value.authors, ...value.value.artists, value.value.uploader])].sort().join(", ")
})
...
<template>
  <div class="bg-white dark:bg-[#191A1C] w-full h-full overflow-auto"
       style="--md-background: 255 255 255; --navbar-height: 2rem; --main-color: 67 45 215">
    <!--<div class="dark bg-white dark:bg-[#191A1C] w-full h-full" style="--md-background: 25 26 28; --navbar-height: 2rem;">-->
    <div style="height:var(--navbar-height)"/>
    <nav class="fixed top-2 left-0 w-full flex items-center justify-between p-4 bg-transparent z-10 h-0">
      <button @click="$router.back()" class="text-white -translate-x-3 -translate-y-2 cursor-pointer">
        <LucideChevronLeft/>
      </button>

      <div class="flex gap-4 ">
        <button class="text-red-500 cursor-pointer" @click="() => value ? (value.favorite = !value.favorite): null">
    ...
    <div v-if="value && titles" class="layout-container manga has-gradient px-4">
      <div v-if="width >= 640" class="absolute top-0 left-0 z-0 w-full h-[640px] blur-xl"
      ...
      <div class="banner-container z-0 top-0">
        <div class="banner-image" :style="`background-image: url('${img_url}'); width: 100%;`"/>
        <div class="banner-shade"/>
      </div>
      <div style="grid-area: art;">
        <div>
          <a class="group flex items-start relative mb-auto select-none " :href="img_url" target="_self">
            <div
              ...
              <svg class="icon xLarge text-white" fill="none" height="24" viewBox="0 0 24 24" width="24"
            ...
            <CoverGet :status="value.status" class-list="rounded shadow-md w-full h-auto" :manga_id="value.manga_id"
                      :ext="value.cover_ext[0] ?? ''"/>
          </a>
        </div>
      </div>
      <div class="title text-white"><p
          class="mb-1"
          style="line-height: 1.1em; overflow-wrap: break-word; text-shadow: rgba(0, 0, 0, 0.3) 1px 2px 4px; font-size: 2rem; width: 800px;">
        {{ titles.title }}</p>
        <div class="font-normal line-clamp-2 text-base sm:text-xl inline-block leading-5"
             title="{{titles.other}}">
          {{ titles.other }}
        </div>
        <div class="grow hidden sm:block"></div>
        <div class="flex flex-row gap-2">
          <div class="font-normal text-xs sm:text-base truncate">{{ creator_string }}</div>
        </div>
      </div>
      ...
      <div class="sm:mx-2 z-1"
           style="grid-area: info;">
        <div class="flex gap-1 flex-wrap items-center">
          <div class="flex flex-wrap gap-1 tags-row"
               style="max-height: calc(1em + 0rem);">
            <a
                v-for="tag in value?.tags"
                class="tag bg-accent"
                href="https://mangadex.com/tag/391b0423-d847-456f-aff0-8b0cfc03066b/action">{{ tag.tag }}</a>
          </div>
        </div>
      </div>
        ...
        <div style="word-break: break-word;">
          <div
            ...
            <div>
              ...
              <SynopsisAllTags class="xl:hidden" :titles="value.titles" :sources="value.sources" :tags="value.tags" :artists="value.artists" :authors="value.authors" :publishers="value.publishers"/>
            </div>
          </div>
        </div>
      </div>
      <div class="mb-8 z-10" style="grid-area: content">
        <div class="flex">
          <div v-if="width >= 1280" class="flex-1/4" >
            <SynopsisAllTags  :titles="value.titles" :sources="value.sources" :tags="value.tags" :artists="value.artists" :authors="value.authors" :publishers="value.publishers"/>
          </div>
          <div :class="width >= 1280 ? 'flex-3/4' : 'flex-1'">
            <button @click="() => ascending = !ascending">{{ ascending ? 'Ascending' : 'Descending' }}</button>
            <ul class="chapter-list">
              <li @click="() => navigateTo(chapter_link(chapter.id))" v-for="chapter in chapters" :key="chapter.id">
...
.md-md-container tr:nth-child(odd) {
  background-color: rgb(var(--md-background)/.6)
}
...
.layout-container.manga {
  --banner-overlay-gradient: linear-gradient(to bottom, rgb(var(--md-background)/.8) 0%, rgb(var(--md-background)) 100%);
  gap: .75rem 1rem;
...
@media not (min-width: 40rem) {
  .layout-container.has-gradient:not(.manga) {
    --banner-overlay-gradient: linear-gradient(to bottom, rgb(var(--md-background)) 0%, rgb(var(--md-background)/.8) 56px, rgb(var(--md-background)/0) 50%)
  }
}
