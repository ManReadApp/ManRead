...
<template>
  <div class="w-full h-80 flex flex-col m-2 my-8">
    <div class="w-full flex justify-between space-x-2 px-3 py-1">
      ...
      <div>
        <button v-if="!no_more" @click="() => navigateTo(route)"
    ...
    <div class="bg-gray-100 rounded-md w-full h-full p-3">
      <div class="flex overflow-x-auto h-full space-x-2 hide-scroll overflow-hidden rounded-md">
        <div
            v-for="item in data"
            :key="item.manga_id"
        ...
        >
          <CoverGet :manga_id="item.manga_id" :ext="item.ext" :status="item.status" class-list="w-full h-[calc(100%-3rem)] object-cover"/>
          <div class="flex items-center justify-center h-[3rem] mx-1  cursor-pointer" @click="() => navigateTo(`/info/${item.manga_id}`)">
            <p class="text-center text-gray-900 line-clamp-2 text-sm font-semibold text-ellipsis">{{
